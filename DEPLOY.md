...
├── DEPLOY.md               этот файл
├── .nojekyll
├── 404.md                  страница «не найдено»
├── guide/                  установка и первый запуск
├── assets/
│   ├── style.css           тема сайта целиком
│   ├── favicon.svg
│   └── screens/            скриншоты экранов приложения
│       ├── *.jpg           настоящие скриншоты
│       ├── flow.svg        схема обмена с сервером (не скриншот)
│       └── _build.py       генератор flow.svg
...
**Добавить страницу:** создайте `.md`-файл и впишите ссылку на него в `sidebar.md`. Ссылки в меню и внутри страниц пишутся от корня сайта (`guide/install.md`), а не относительно текущего файла — за это отвечает `relativePath: false` в `index.html`.
...
**Обновить скриншоты:** замените файл в `assets/screens/*.jpg` — это настоящие снимки экрана с телефона, обрезанные по ширине ~560px. Сжимайте перед заменой, чтобы страница не тяжелела (`sips --resampleWidth 560 -s formatOptions 78 исходник.jpg --out assets/screens/имя.jpg`).

**Перерисовать схему обмена с сервером:** отредактируйте `assets/screens/_build.py` и запустите его — он перезапишет `flow.svg`. Нужен только Python 3, сторонних библиотек нет.
...
Скриншот рядом с текстом:

```html
<div class="pair">
<img src="assets/screens/settings.jpg" alt="Описание для незрячих">
<div>

Текст справа от скриншота.
...
При выходе новой версии приложения обновите номер версии в `index.html` (блок `sidebar-footer`) и в `README.md`, а также имя файла APK в `guide/install.md`.
